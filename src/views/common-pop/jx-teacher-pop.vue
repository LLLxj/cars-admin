<template>
  <el-dialog title="讲师" :close-on-click-modal="false" :visible.sync="visible" append-to-body width="500px">
...
<script>
  import { jxTeacherList } from '@/api/sys'
  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        jxTeacherList(this.pageIndex, this.pageSize, this.dataForm.username).then(res => {
      ...
      selectHandle () {
        ...
        this.$emit('jx-pop-event', this.dataForm.userId, this.dataForm.username)
        this.visible = false
      },
      // 取消
      cancleHandle () {
