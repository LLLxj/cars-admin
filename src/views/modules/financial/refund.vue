        ...
        <el-table-column prop="remark" header-align="center" align="center" label="备注">
          ...
          <el-table-column fixed="right" header-align="center"  align="center"  width="150"  label="操作">
            <template slot-scope="scope">
              ...
              <el-button v-if="isAuth('deal:user:store:refund:update') && scope.row.status === 1 || scope.row.status === 2" type="text" size="small" @click="addOrUpdateHandle(null, scope.row.refundId)">编辑</el-button>
              <el-button v-if="isAuth('deal:user:store:refund:reject') && scope.row.status === 3 || scope.row.status === 2" type="text" size="small" @click="checkOrder(scope.row.refundId, 1)">驳回</el-button>
              <el-button v-if="isAuth('deal:user:store:refund:manager') && scope.row.status === 2" type="text" size="small" @click="checkOrder(scope.row.refundId, 2)">经理审核</el-button>
              <el-button v-if="isAuth('deal:user:store:refund:success') && scope.row.status === 3" type="text" size="small" @click="checkOrder(scope.row.refundId, 3)">通过</el-button>
