<template>
    <el-dialog title="班级列表" :close-on-click-modal="false" :visible.sync="visible" append-to-body>
        <el-form :inline="true" :model="dataCplan" @keyup.enter.native="getDataList()">
          <el-form-item>
            <el-input v-model="dataCplan.cplanName" placeholder="请输入班级计划名称" clearable></el-input>
          </el-form-item>
          <el-form-item>
      ...
      @row-dblclick="handleRowDblclick" @row-click="handleRowlclick" style="width: 100%;">
          ...
          <el-table-column prop="courseName" header-align="center" align="center" label="科目"/>
  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        cplanList(this.pageIndex, this.pageSize, this.dataForm.cplanName).then(res => {
      ...
      selectHandle () {
        ...
        this.$emit('cplan-pop-event', this.dataCplan.cplanId, this.dataCplan.cplanName)
        this.visible = false
      },
      // 取消
      cancleHandle () {
