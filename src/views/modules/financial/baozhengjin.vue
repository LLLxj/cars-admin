        ...
        <el-table :data="dataList" border stripe v-loading="dataListLoading" @selection-change="selectionChangeHandle" style="width: 100%;" id="dataListUser">
          ...
          <el-table-column fixed="right" header-align="center"  align="center"  width="150"  label="操作">
            <template slot-scope="scope">
              ...
              <el-button v-if="isAuth('deal:user:store:deposit:update') && scope.row.status === 2 || scope.row.status === 1" type="text" size="small" @click="addOrUpdateHandle(null, scope.row.depositId)">编辑</el-button>
              <el-button v-if="isAuth('deal:user:store:deposit:reject') && scope.row.status === 2 || scope.row.status === 3" type="text" size="small" @click="checkOrder(scope.row.depositId, 1)">驳回</el-button>
              <el-button v-if="isAuth('deal:user:store:deposit:manager') && scope.row.status === 2" type="text" size="small" @click="checkOrder(scope.row.depositId, 2)">经理审核</el-button>
              <el-button v-if="isAuth('deal:user:store:deposit:success') && scope.row.status === 3" type="text" size="small" @click="checkOrder(scope.row.depositId, 3)">通过</el-button>
...
<script>
  ...
  import AddOrUpdate from '@/views/modules/customer/user-ensure-money'
