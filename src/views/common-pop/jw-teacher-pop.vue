<template>
  <el-dialog title="教务老师" :close-on-click-modal="false" :visible.sync="visible" append-to-body>
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      <el-form-item>
        <el-input v-model="dataForm.username" placeholder="老师名" clearable></el-input>
      </el-form-item>
      <el-form-item>
    ...
    <el-table :data="dataList" border v-loading="dataListLoading" highlight-current-row @row-dblclick="handleRowDblclick" @row-click="handleRowlclick" style="width: 100%;">
      ...
      <el-table-column prop="username" header-align="center" align="center" label="老师姓名">
      </el-table-column>
...
<script>
 ...
 import { jwTeacherList } from '@/api/sys'
  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        jwTeacherList(this.pageIndex,this.pageSize,this.dataForm.username).then(res => {
      ...
      selectHandle () {
        ...
        this.$emit('jw-pop-event', this.dataForm.userId, this.dataForm.username)
        this.visible = false
      },
      // 取消
      cancleHandle () {
