<template>
  <el-dialog title="审核人" :close-on-click-modal="false" :visible.sync="visible" append-to-body width="600px">
...
<script>
   import { getAllCheckList } from '@/api/sys'
  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        getAllCheckList(this.pageIndex, this.pageSize, this.dataForm.username).then(res => {
      ...
      selectHandle () {
        ...
        this.$emit('all-check-pop-event', this.dataForm.userId, this.dataForm.username)
        this.visible = false
      },
      // 取消
      cancleHandle () {
