<template>
  <el-dialog title="电邀老师" :close-on-click-modal="false" :visible.sync="visible" append-to-body width="500px">
...
<script>
   import { tgTeacherList } from '@/api/sys'
  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        tgTeacherList(this.pageIndex, this.pageSize, this.dataForm.username).then(res => {
      ...
      // 确认
      selectHandle () {
        if (this.dataForm.userId == '' || this.dataForm.userId == undefined) {
          this.$message.error('电邀老师不能为空!')
          return
        }
        this.$emit('tg-pop-event', this.dataForm.userId, this.dataForm.username)
        this.visible = false
      },
      // 取消
      cancleHandle () {
