      ...
      <el-main>
        ...
        <el-table :data="dataList" border stripe v-loading="dataListLoading" @selection-change="selectionChangeHandle" style="width: 100%;" id="dataListUser">
          ...
          <el-table-column fixed="right" header-align="center"  align="center"  width="150"  label="操作">
            <template slot-scope="scope">
              <el-button type="text" size="small" v-if="isAuth('conf:advertising:disable') && scope.row.status === 1" @click="disHandle(scope.row)">禁用</el-button> 
              <el-button type="text" size="small" v-if="isAuth('conf:advertising:normal') && scope.row.status === 0" @click="norHandle(scope.row)">启用</el-button>
              <el-button v-if="isAuth('conf:advertising:update')" type="text" size="small" @click="addOrUpdateHandle(scope.row.adId)">编辑</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
