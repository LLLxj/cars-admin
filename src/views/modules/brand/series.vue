      ...
      <el-main>
        ...
        <el-table :data="dataList" border stripe v-loading="dataListLoading" style="width: 100%;" id="dataListUser">
          ...
          <el-table-column fixed="right" header-align="center"  align="center"  width="150"  label="操作">
            <template slot-scope="scope">
              <el-button v-if="isAuth('cou:wares:series:update')" type="text" size="small" @click="addOrUpdateHandle(scope.row.couSeriesId)">编辑</el-button>
              <el-button type="text" size="small" v-if="isAuth('cou:wares:series:disable') && scope.row.status === 1" @click="disHandle(scope.row.couSeriesId)">禁用</el-button> 
              <el-button type="text" size="small" v-if="isAuth('cou:wares:series:normal') && scope.row.status === 0" @click="norHandle(scope.row.couSeriesId)">启用</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
