  ...
  <div>
    <el-dialog  :close-on-click-modal="false" :visible.sync="visible" width="80%" top="15vh">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button v-if="isAuth('stu:reg:refund:update')" @click="refuse()">拒绝</el-button>
        <el-button v-if="isAuth('stu:reg:refund:update')" type="primary" @click="accept()">同意</el-button>
      </span>
    </el-dialog>
  </div>
  <!-- 弹窗部分 -->
...
<script>
  ...
  import { refundInfo, refundList, refuseRefund, acceptRefund } from '@/api/refund'
    ...
    methods: {
      ...
      accept () {
        this.$confirm(`确定同意?`, '提示', {
          ...
          acceptRefund(this.refundId).then(res => {
            if (res.data && res.data.code === 0) {
              this.$message({
                message: '操作成功',
                type: 'success',
      ...
      refuse () {
        this.$confirm(`确定拒绝?`, '提示', {
          ...
          refuseRefund(this.refundId).then(res => {
            if (res.data && res.data.code === 0) {
              this.$message({
                message: '操作成功',
                type: 'success',
